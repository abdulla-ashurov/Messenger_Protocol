...
void setFlag(std::vector<uint8_t>::iterator it) {
    const uint8_t defaultFlag = 0x5;
    *it = defaultFlag;
    *it <<= 5;
}

void setNameLength(std::vector<uint8_t>::iterator it, const uint8_t length) {
    uint8_t temp = length << 1;
    *it |= temp;
}

void setMsgLength(std::vector<uint8_t>::iterator it, const uint8_t length) {
...
void setHeader(std::vector<uint8_t>::iterator it, const uint8_t name_length, const uint8_t msg_length, const uint8_t crc4) {
    setFlag(it);
    setNameLength(it, name_length);
    setMsgLength(it, msg_length);
    setCRC4(it + 1, crc4);
}

void setSenderName(std::vector<uint8_t>::iterator begin, std::vector<uint8_t>::iterator end, const std::string &name) {
